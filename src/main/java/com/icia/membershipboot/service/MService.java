...
import com.icia.membershipboot.dao.MDAO;
...
public class MService {
	...
	public ModelAndView mJoin(MEMBER member) throws IllegalStateException, IOException {
		...
		Path path = Paths.get(System.getProperty("user.dir"), "membershipBoot/src/main/resources/static/profile");

			memProfile.transferTo(new File(path+"/"+profileName));
	...
	public String idoverlap(String memId) {
		System.out.println("중복체크 서비스로 넘어옴??"+memId);
		String checkId = mdao.idCheck(memId);
		System.out.println("DB갔다온건가??"+memId);
		String result;
		if (checkId == null) {
			// 아이디 사용가능
			result = "OK";
